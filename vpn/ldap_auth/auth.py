"""
This module provides an authentication backend that relies on LDAP directory to get users and to check machines permissions.

.. seealso:: Authentication backends for handling machine permissions are specified here :mod:`vimma.vmm.auth.VMMBackend`.
"""
...
class RemoteUserLDAPBackend(RemoteUserBackend):
    """
    This authentication backend relying on :class:`RemoteUserBackend`, uses LDAP to authenticate a user, and check its rights on a machine.
    
    .. seealso:: The `django doc <http://docs.djangoproject.com/en/dev/howto/auth-remote-user/>`_ about :class:`RemoteUserBackend`.
    """
    
    def configure_user(self, user):
        """
        Set user's e-mail address in the database from LDAP. Also give superuser rights if the user is in TeamIt group in LDAP.
    ...
    def get_user(self, username):
        ...
        Verify if a user with `username` is in the LDAP directory :
            - if it is, the method gets or creates this user in the database and returns it. If the user has to be created, the :meth:`RemoteUserLDAPBackend.configure_user` will be called on the newly created user.
