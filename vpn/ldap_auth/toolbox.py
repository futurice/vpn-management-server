"""
The module :mod:`ldap_auth.toolbox`, is a toolbox of functions that allow to
realize various operations (related to vmm) in the ldap directory.

In order for this module to work correctly, here are the settings that have to
be defined :
...
.. todo:: this is no very robust. Would deserve more verifications, better
testing and better error handling ; if it happen more used than now...
"""
...
def get_binded_connection():
    """
    Returns a binded LDAP connection, with the LDAP parameters in defined in the
    settings.
    """
...
def get_user(username):
    """
    Returns a dictionnary containing the user's attributes as recorded in the
    LDAP directory ; or None if no user with such a username could be found.
    ...
    user = con.search_s("%s,%s" % (settings.USERS_OU, settings.DC),
                        ldap.SCOPE_ONELEVEL,
                        "(&(objectClass=inetOrgPerson)(uid=%s))" % username)
    con.unbind_s()
    if user:
        return user[0][1]
    else:
...
def get_sudoers(machine_name):
    ...
    sudoers = con.search_s("%s,%s" % (settings.ADMINS_OU, settings.DC),
                           ldap.SCOPE_ONELEVEL,
                           "(&(objectClass=posixGroup)(uniqueMember=uid=*,%s,%s)(cn=%s))"
                           % (machine_name, settings.USERS_OU, settings.DC, ["uniqueMember"]))
    con.unbind_s()
    if sudoers:
...
def get_project_groups(keyword):
    """
    This function returns a list of project groups that contains `keyword`,
...
def get_usernames(keyword):
    """
    This function returns a list of usernames that contains `keyword`,
    from the LDAP directory.
    """
    con = get_binded_connection()
...
def get_admin_usernames():
    ...
    users = con.search_s("%s,%s" % (settings.TEAMS_OU, settings.DC),
                         ldap.SCOPE_SUBTREE, "(cn=" + settings.ADMIN_TEAM_NAME + ")",
                         ['uniqueMember'])[0][1]["uniqueMember"]
...
def _unicode_to_str(a_str):
    """
    Transtype the unicode str passed as a parameter to a normal str.
    If the parameter is not a unicode, it returns it as it is.
    """
